...
std::vector<std::pair<int, std::pair<int, int>>> CentroidTracker::update(std::vector<std::vector<int>> boxes) {
    ...
    std::vector<pair<int, int>> inputCentroids;
    for (auto b : boxes) {
    ...
    if (this->objects.empty()) {
        ...
        std::vector<pair<int, int>> objectCentroids;
...
//        Calculate Distances
        ...
        for (uint i = 0; i < objectCentroids.size(); ++i) {
            std::vector<float> temp_D;
            for (std::vector<std::vector<int>>::size_type j = 0; j < inputCentroids.size(); ++j) {
        ...
        std::vector<pair<float, int>> temp_rows;
        int k = 0;
        for (auto i: D_copy) {
        ...
        for (uint i = 0; i < rows.size(); i++) {
            ...
            for (uint t = 0; t < this->objects.size(); t++) {
        ...
        //D.shape[1]
        for (uint i = 0; i < inputCentroids.size(); i++) {
            inpCols.insert(i);
        }
    ...
    if (!objects.empty()) {
        for (auto obj: objects) {
            ...
            if (path_keeper[obj.first].size() > 5) {
