...
int parse_args(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "w:h:l:f:t:d:s:r:v", long_options, &option_index))) {
		...
		switch (c) {
			...
			case 'r':
				...
				break;
			case 'v':
				// std::cout << "\t\tVersion: e-multicam.elf-v" << VERSION << std::endl;
				device_data.exit_flag = 1;
				break;
			case 's':
	...
	if(cmdline.record == 1 )
	std::cout << RED("################# Video Recording in Progress ... ####################\n")RESET << std::endl;
	if (device_data.exit_flag != 1) {
		// Resize frames based on the size of the display.
		if (def_w != 0 || def_h != 0) {
			std::cout << "SETTING DEFAULT RESOLUTION\n";
	...
	if (cmdline.width > WIDTH_UHD || cmdline.height > HEIGHT_UHD){
		std::cout << "RESOLUTION NOT SUPPORTED\n";
		device_data.exit_flag = 1;
	}
	return 0;
}
...
{  
	...
	ECAL_HANDLE pub     = 0;
  	...
  	int         sent    = 0;
	eCAL_Initialize(argc, argv, "e-multicam", eCAL_Init_Default);
	pub = eCAL_Pub_New();
